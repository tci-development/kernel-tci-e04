...
#pragma GCC diagnostic pop
	...
	if (suspend) {
		...
		if (!dhdinfo->pub.early_suspended)
		...
		if (dhdinfo->pub.conf->suspend_mode == PM_NOTIFIER ||
				dhdinfo->pub.conf->suspend_mode == MIX_SUSPEND)
...
static void dhd_early_suspend(struct early_suspend *h)
{
	...
	if (dhd && (dhd->pub.conf->suspend_mode == EARLY_SUSPEND ||
			dhd->pub.conf->suspend_mode == MIX_SUSPEND)) {
...
#define EVENT_LOG_RATE_HC_THRESHOLD	1000
	...
	ret = rk_vendor_read(WIFI_MAC_ID, addr, ETHER_ADDR_LEN);
	if (ret != ETHER_ADDR_LEN || is_zero_ether_addr(addr)) {
		DHD_ERROR(("%s: rk_vendor_read wifi mac address failed (%d)\n", __FUNCTION__, ret));
		DHD_ERROR(("%s: write wifi mac address to rk vendor\n", __FUNCTION__));
		ret = rk_vendor_write(WIFI_MAC_ID, buf, ETHER_ADDR_LEN);
		if (ret != 0) {
			DHD_ERROR(("%s: rk_vendor_write failed %d\n", __FUNCTION__, ret));
		}
	}
	else if(memcmp(buf, addr, ETHER_ADDR_LEN) != 0) {
		DHD_ERROR(("%s: wifi mac address in rk_vendor error\n", __FUNCTION__));
		DHD_ERROR(("%s: write wifi mac address to rk vendor\n", __FUNCTION__));
		ret = rk_vendor_write(WIFI_MAC_ID, buf, ETHER_ADDR_LEN);
		if (ret != 0) {
			DHD_ERROR(("%s: rk_vendor_write failed %d\n", __FUNCTION__, ret));
		...
		DHD_ERROR(("%s: wifi mac address in rk_vendor ready\n", __FUNCTION__));
...
static int wifi_init_thread(void)
{
	dhd_module_init();
...
static void wifi_exit_thread(void)
{
	dhd_module_exit();
}
#if 1
...
int net_os_set_suspend(struct net_device *dev, int val, int force)
{
	...
	if (dhd && dhd->pub.conf->suspend_mode == EARLY_SUSPEND && !val)
