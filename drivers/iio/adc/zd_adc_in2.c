...
static DEVICE_ATTR(get_adc_in2_value, 0664, adc_inx_value_show, adc_inx_value_store);

static const struct of_device_id zd_adcins_match[] = {
	{ .compatible = "zd,adc_in2", .data = NULL},
	{},
};
...
static int adcins_probe(struct platform_device *pdev)
{
	...
	error = device_create_file(dev, &dev_attr_get_adc_in2_value);
	if(error)
	{
