						...
						const char *buf, size_t size)
{
	...
	if (val== 1) { 
		...
		printk("-zd-: _%s_ :gpio pull up\n", __func__);
		gpio_direction_output(gpio5_b2, val); 

	} else if (val == 0) { 

		printk("-zd-: _%s_ :gpio pull down\n", __func__);
		gpio_direction_output(gpio5_b2, val); 
...
MODULE_AUTHOR("zd,chris.zhang@zunidata.com");
