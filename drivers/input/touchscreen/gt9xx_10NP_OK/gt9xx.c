...
//                u8 type = ((doze_buf[2] & 0x0F) - 0x0A) + (((doze_buf[2] >> 4) & 0x0F) - 0x0A) * 2;
	...
	if( DOZE_ENABLED==doze_status )
	{
		...
		touch_num = 0;//@R add. 20191101
		//return;
		goto exit_work_func;//@R add. 20191101
	} 

	if(ts->gtp_is_suspend)  //@R add. 20191101
	{
        goto exit_work_func;
    }
...
#if GTP_ICS_SLOT_REPORT
	#if !defined(CONFIG_LCD_TSH070AI01)	//@ZD 10"
...
#if defined(CONFIG_ZND_OUT_INTERFACE_MIPI)	// ian : 10V-RN
	#if defined(CONFIG_LCD_LKW101IH27810188A)	// Link well
			m89or101 = FALSE;
			gtp_change_x2y = TRUE;
			gtp_x_reverse = TRUE;
			gtp_y_reverse = FALSE;
